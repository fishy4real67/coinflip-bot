...
async def coinflip(
    ...
    # UPDATE STATS
    get_stats(winner.id)["wins"] += 1
    get_stats(loser.id)["losses"] += 1
...
# STATS COMMAND
# -------------------------
@bot.tree.command(name="stats", description="View your coinflip stats")
@app_commands.describe(member="User (optional)")
async def stats_cmd(interaction: discord.Interaction, member: discord.Member = None):
    ...
    embed = discord.Embed(title="📊 Coinflip Stats", color=discord.Color.blue())
    embed.add_field(name="User", value=user.mention, inline=False)
    embed.add_field(name="Wins", value=str(wins))
    embed.add_field(name="Losses", value=str(losses))
    embed.add_field(name="Total Games", value=str(wins + losses))
...
# LEADERBOARD COMMAND
# -------------------------
@bot.tree.command(name="leaderboard", description="Top players by wins")
async def leaderboard(interaction: discord.Interaction):

    if not stats:
        return await interaction.response.send_message("No games played yet.")

    sorted_players = sorted(stats.items(), key=lambda x: x[1]["wins"], reverse=True)

    embed = discord.Embed(title="🏆 Coinflip Leaderboard", color=discord.Color.gold())

    text = ""
    for i, (user_id, data) in enumerate(sorted_players[:10], start=1):
        try:
            user = await bot.fetch_user(user_id)
            text += f"**#{i}** {user.name} — {data['wins']} wins\n"
        except:
            continue
